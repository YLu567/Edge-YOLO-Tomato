...
class Partial_conv3(nn.Module):
    def __init__(self, dim, n_div=4, forward='split_cat'):
        ...
        # 复用已有卷积核，不新增参数
        self.partial_conv3 = nn.Identity()  # 替换原始卷积为恒等映射
...
class Faster_Block(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        # 确保输入输出通道一致
        assert inc == dim, "Input and output channels must be equal"

        self.mlp_ratio = mlp_ratio
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()

        # 简化MLP结构（参数复用）
        self.mlp = nn.Identity()  # 移除额外参数
        ...
        # 固定层缩放系数
        self.register_buffer('layer_scale', torch.ones((dim)))

    def forward(self, x):
        # 残差连接保持维度不变
        return x + self.drop_path(self.spatial_mixing(x))
...
class C2f_Faster(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        # 使用参数共享的Faster Block
        shared_block = Faster_Block(self.c, self.c)
        self.m = nn.ModuleList([shared_block for _ in range(n)])
